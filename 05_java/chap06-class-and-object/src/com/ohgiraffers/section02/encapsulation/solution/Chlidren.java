...
public class Chlidren {
    ...
    * (#) protected : 통일 패키지에서 접근 허용 (단, 상속 관계에 있는 경우 다른 패키지에서도 접근 가능)
    * (~) default : 동일 패키지 내에서만 접근 허용 (작성하지 않는것이 default)
    * (-) private : 해당 클래스 내부에서만 접근 허용
    *
    * 위 네가지 접근제한자는 클래스, 생성자, 클래스의 멤버(필드, 메소드)에 모두 사용가능하다.
    * 단, 클래스 선언 시 사용하는 접근제한자는 public과 defult만 사용 가능하다.
    * */

    /*
    * [캡슐화]
    * 유지보수성을 증가시키기 위해 필드의 직접 접근을 제한하고
    * public 메소드(getter, setter)를 이용해 접근하여 사용할 수 있도록 만든 기술
    ...
    * */
    ...
    /*
    * [Getter와 Setter 메소드]
    * Getter(접근자) : 내부 내부 필드의 값을 외부로 읽어서(Get) 전달(반환)하는 역할을 하는 메소드(읽기 전용)
    ...
    * */
    ...
    public void setAge(int age){
        if (age >=0){
            this.age = age; // this : 현재 겍체 자신을 가리키는 참조 변수
        }
        else {
