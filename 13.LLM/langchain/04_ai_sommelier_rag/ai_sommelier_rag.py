...
def search_wines(query):
    ...
    Returns:
        dict: 다음 단계로 넘겨줄 데이터 셋 (요리 묘사 + 검색된 와인 리뷰둘)
    """
    ...
    # 유사도 검색 실행
    results = vector_db.similarity_search(query, k=5, namespace=os.getenv("PINECONE_NAMESPACE"))
...
def recommend_wines(query):
    ...
    와인페어링 추천에 아래의 요리와 풍미, 와인리뷰만을 참고하여 답변해주세요.
     요리와 풍미:
    {dish_flavor}
     와인리뷰:
    {wine_reviews}
